...
public:
	...
	void remove(int key, int from = 0, string fileName = "", bool is_Binary = true) {
		fileName = fileName.size() ? fileName : this->fileName;
		...
		file.remove(num, fileName, is_Binary);
		sync(fileName, is_Binary);
	}
	...
	void sync(string fileName = "", bool isBinary = true) {
		fileName = fileName.size() ? fileName : this->fileName;
		table.clear();
		init_table(fileName, isBinary);
	}
};